...
class GameScene extends Phaser.Scene {
  ...
  update() {
    ...
    // console.log(`Timer Meter Body Max: ${gameState.currentCustomer.meterBase.width}`);
    // console.log(`Timer Meter Body Length: ${gameState.currentCustomer.timerMeterBody.width}`);
    // console.log(`Test variable length for timer reduction: ${gameState.currentCustomer.meterBase.width - (gameState.timer.getProgress() * gameState.currentCustomer.meterBase.width)}`);
    // console.log(`Game Speed: ${gameState.gameSpeed}`);
...
/*
Custom helper scripts below: Used for updating customer count text, placing food, tracking fullness,
moving the customers down the line, and calculating stars for service.
*/
  ...
  placeFood(food, fullnessValue) {
    if (gameState.currentMeal.children.entries.length < 3 && gameState.customerIsReady === true) {
      ...
      // console.log(`Current fullness: ${gameState.currentMeal.fullnessValue}/${gameState.currentCustomer.fullnessCapacity}`);
      ...
      for (let i = 0; i < gameState.currentMeal.fullnessValue; i++) {
        ...
        /*
        // Note: The following loop needs to be made because iterator i exceeds the array length of fullnessMeterBlocks;
        this is specifically to scope down to the length of the array to new iterator j, which is the exact length of
        fullnessMeterBlocks.
        */
       if (gameState.currentMeal.fullnessValue > gameState.currentCustomer.fullnessCapacity) {
         for (let j = 0; j < gameState.currentCustomer.fullnessMeterBlocks.length; j++) {
          ...
          /* CONSOLE TESTING
         console.log(`Exceeded Hunger; ${gameState.currentCustomer.fullnessMeterBlocks.length}/${gameState.currentMeal.fullnessValue}.`);
         */
         }
       }
      }
